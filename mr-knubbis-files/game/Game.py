...
class Enemy(pygame.sprite.Sprite):
    game = None
    def __init__(self, game, offset=0):
        super().__init__()
        self.game = game
        self.image = pygame.image.load("assets\\mr-knubbis\\textures\\player-enemy\\Enemy.png")
        self.image = pygame.transform.scale(self.image, (64, 64))
        self.surf = pygame.Surface((42, 70))
        self.rect = self.surf.get_rect(center = (self.game.SCREEN_WIDTH + randint(5,30)*20, self.game.SCREEN_HEIGHT - 40))
    ...
    def move(self):
        ...
        if (self.rect.left < 0):
            self.game.score += 1
            self.rect.right = 0
            self.rect.center = (self.game.SCREEN_WIDTH + randint(5,30)*20, self.game.SCREEN_HEIGHT - 40)
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, game):
        ...
        self.rect = self.surf.get_rect(center = (40, self.game.SCREEN_HEIGHT - 40))
...
class Knubbis:
    ...
    def update_highscore_label(self, widget, menu):
        widget.set_title("Highscore: %s" % (self.highscore))
    ...
    def start(self):
        ...
        mytheme.widget_url_color = (50, 50, 50)

        self.log("Creating about menu")
        about_menu = pygame_menu.Menu("About", self.SCREEN_WIDTH, self.SCREEN_HEIGHT, theme=mytheme )
        about_menu.add.url('https://github.com/KnubbHub', 'GitHub')
        about_menu.add.button('Back', pygame_menu.events.BACK)

        self.log("Creating settings menu")
        settings_menu = pygame_menu.Menu("Settings", self.SCREEN_WIDTH, self.SCREEN_HEIGHT, theme=mytheme )
        ...
        highscore_menu = pygame_menu.Menu("Highscore", self.SCREEN_WIDTH, self.SCREEN_HEIGHT, theme=mytheme )
        highscore_label = highscore_menu.add.label("Highscore: ...")
        highscore_label.add_draw_callback(self.update_highscore_label)
        highscore_menu.add.button('Back', pygame_menu.events.BACK)

        self.log("Creating main menu for the first time.")
        menu = pygame_menu.Menu("Mr: Knubbis", self.SCREEN_WIDTH, self.SCREEN_HEIGHT, theme=mytheme )
    ...
    def run_game(self):
        ...
        P1 = Player(self)
        if self.get_difficulty() == 0.1:
            E1 = Enemy(self)
            E2 = Enemy(self, 100)
        if self.get_difficulty() == 0.3:
            E1 = Enemy(self)
            E2 = Enemy(self, 100)
            E3 = Enemy(self, 200)
        if self.get_difficulty() == 1.0:
            E1 = Enemy(self)
            E2 = Enemy(self, 100)
            E3 = Enemy(self, 200)
            E4 = Enemy(self, 300)
